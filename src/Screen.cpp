...
void Screen::boxBlur() {
	...
	for(int y = 0; y < SCREEN_HEIGHT; y++) {
		...
		for(int x = 0; x < SCREEN_WIDTH; x++) {
			...
			/*
			 * Box Blur Effect: Adding the color values (R, G, B) of the all the nine pixels to the central pixel.
			 * 0 0 0
			 * 0 1 0
			 * 0 0 0
			 */
...
void Screen::setPixel(int x, int y, Uint8 red, Uint8 green, Uint8 blue) {
	...
	// Color Format of the Pixel: R G B A
	color += red;
	color <<= 8;
	color += green;
	color <<= 8;
	color += blue;
	color <<= 8;
	color += 0xFF; // Alpha Opaque Value (255)
